...
// --- Begin: Customers Private Dining Schema --- //
const schema = new Schema({
  ...
  // Contact Detatils
  bookingId: {type: String, required: true, index: true, unique: true}, // (CurrYear-2022) + DayofYear + CurrTime in Seconds + Last 4 digits of Mobile Number
...
schema.index({idSeq: 'text', bookingId: 'text', name: 'text', mobCcNum: 'text', emID: 'text', bStatus: 'text', bDtStr: 'text', location: 'text'});
schema.index({user: 1, mobCcNum: 1, emID: 1});
schema.index({idSeq: 1, branch: 1, user: 1, delFlag: -1, bStatus: 1});
schema.index({cDtTm: -1, cDtStr: -1, uDtTm: -1, uDtStr: -1});

module.exports = mongoose.model(config.collCustsPrivateDining, schema);
// --- End: Customers Private Dining Schema --- //
