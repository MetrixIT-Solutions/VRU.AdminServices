...
// --- Begin: Customers Users GSI(Guest Satisfaction Index) Closed Schema --- //
...
}, {collection: config.collCustsUsersGSIClsd});

schema.index({branch: 1, sDtStr: 1, gMob: 1}, {unique: true});
schema.index({branch: 1, delFlag: -1});
schema.index({cDtStr: -1});

module.exports = mongoose.model(config.collCustsUsersGSIClsd, schema);
// --- End: Customers Users GSI(Guest Satisfaction Index) Closed Schema --- //
