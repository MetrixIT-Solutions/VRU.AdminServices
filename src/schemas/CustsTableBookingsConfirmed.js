...
// --- Begin: Customers Table Bookings Confirmed Schema --- //
const schema = new Schema({
  ...
  bStatus: {type: String, required: true}, // Booking Status: Confirmed, Waiting, Seated
...
}, {collection: config.collCustsTableBookingsConfirmed});

schema.index({idSeq: 'text', bookingId: 'text', name: 'text', mobCcNum: 'text', emID: 'text', occassion: 'text', bDtStr: 'text', location: 'text'});
schema.index({user: 1, mobCcNum: 1, emID: 1});
schema.index({idSeq: 1, branch: 1, user: 1, bStatus: 1, delFlag: -1, offer: 1});
schema.index({cDtTm: -1, cDtStr: -1, uDtTm: -1, uDtStr: -1, bDtStr: 1});

module.exports = mongoose.model(config.collCustsTableBookingsConfirmed, schema);
// --- End: Customers Table Bookings Confirmed Schema --- //
