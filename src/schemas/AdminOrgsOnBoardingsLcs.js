...
// --- Begin: Admin Orgs On-Boardings Lifecycles Schema --- //
const schema = new Schema({
  _id: {type: String, default: uuidv4()},

  orgId: {type: String, required: true}, // Admin Organizations Record ID(_id) - config.collAdminOrgsOnBoardings
  ...
  altEmID: {type: String, required: false},
  doi: {type: String, required: false}, // Date Of Incorporation
  gst: {type: String, required: false}, // GST Number
  pan: {type: String, required: false}, // PAN Number
  cin: {type: String, required: false}, // Corporate Identification Number
  tan: {type: String, required: false}, // Tax Deduction Account Number
  ...
  aprvdDtStr: {type: String, required: false}, // Date & Time - Format = YYYY-MM-DD HH:mm
  rjctdDtStr: {type: String, required: false}, // Date & Time - Format = YYYY-MM-DD HH:mm
...
schema.index({delFlag: -1, orgId: 1});
schema.index({cDtStr: -1});

module.exports = mongoose.model(config.collAdminOrgsOnBoardingsLcs, schema);
// --- End: Admin Orgs On-Boardings Lifecycles Schema --- //
